...
inline std::string IPToName(const std::string& ipAddr)
{
    std::string result;
    ...
    if ((getaddrinfo(ipAddr.c_str(), nullptr, &aiHints, &aiList)) == 0)
    {
        ...
        while (ai != NULL)
        {
            ...
            if (getnameinfo(ai->ai_addr, static_cast<socklen_t>(ai->ai_addrlen), buf, NI_MAXHOST, NULL, 0, NI_NOFQDN | NI_NAMEREQD) == 0)
            {
                result = buf;
            }
            ai = ai->ai_next;
...
public:
  ...
  virtual void get_local_ip_and_port(std::string &ip, int &port, std::vector<uint8_t>* ipaddr) const = 0;
...
inline bool keep_alive(socket_t sock, time_t keep_alive_timeout_sec, bool noTimeout) {
  ...
  while (true) {
    ...
    if (val < 0) {
      ...
      if (!noTimeout && duration.count() > timeout) { return false; }
...
process_server_socket_core(const std::atomic<socket_t> &svr_sock, socket_t sock,
  ...
  while (svr_sock != INVALID_SOCKET && count > 0 &&
    ...
    if (keep_alive_max_count != std::numeric_limits<size_t>::max())
    {
      count--;
    }
  }
...
inline bool get_ip_and_port(const struct sockaddr_storage &addr,
                            socklen_t addr_len, std::string &ip, int &port, std::vector<uint8_t>* ipaddr = nullptr) {
  if (addr.ss_family == AF_INET) 
  {
    ...
    {
      ipaddr->resize(4);
			memcpy(ipaddr->data(), &reinterpret_cast<const struct sockaddr_in *>(&addr)->sin_addr.s_addr, 4);
    }
  } 
  else if (addr.ss_family == AF_INET6) 
  {
    ...
    {
      ipaddr->resize(16);
			memcpy(ipaddr->data(), &reinterpret_cast<const struct sockaddr_in6 *>(&addr)->sin6_addr, 16);
    }
  } 
  else 
  {
...
inline void get_local_ip_and_port(socket_t sock, std::string &ip, int &port, std::vector<uint8_t>* ipaddr) {
  struct sockaddr_storage addr;
  socklen_t addr_len = sizeof(addr);
  if (0 == getsockname(sock, reinterpret_cast<struct sockaddr *>(&addr),
                   &addr_len)) 
  {
    get_ip_and_port(addr, addr_len, ip, port, ipaddr);
  }
}
...
inline bool expect_content(const Request &req) {
  ...
  return req.has_header("Content-Length");
}
...
inline std::string MD5_EX(const std::string &s, bool forceUppercase = false) {
  std::string result = message_digest(s, EVP_md5());
...
inline std::string SHA_256_EX(const std::string &s, bool forceUppercase = false) {
    std::string result = message_digest(s, EVP_sha256());
...
inline std::string SHA_512_EX(const std::string &s, bool forceUppercase = false) {
    std::string result = message_digest(s, EVP_sha512());
...
inline void SocketStream::get_local_ip_and_port(std::string &ip,
                                                int &port, std::vector<uint8_t>* ipaddr) const {
  return detail::get_local_ip_and_port(sock_, ip, port, ipaddr);
}
...
inline void BufferStream::get_local_ip_and_port(std::string & /*ip*/,
                                                int & /*port*/, std::vector<uint8_t>* /*ipaddr = nullptr*/) const {}
...
inline bool Server::is_running() const noexcept { return is_running_; }

inline void Server::wait_until_ready() const noexcept {
...
inline bool Server::write_response_core(Stream &strm, bool close_connection,
  ...
  } else if (keep_alive_max_count_ != std::numeric_limits<size_t>::max()){
    std::stringstream ss;
  ...
  // Response line and headers
  {
    ...
    if (!bstrm.write_format("%s %d %s\r\n", res.version.c_str(), res.status,
...
inline void SSLSocketStream::get_local_ip_and_port(std::string &ip,
                                                   int &port, std::vector<uint8_t>* ipaddr) const {
  detail::get_local_ip_and_port(sock_, ip, port, ipaddr);
}
